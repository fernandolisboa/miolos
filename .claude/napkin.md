...
8. **[2026-08-14] Pre-commit's full suite flakes under turbo's parallel fan-out (PGlite `beforeAll` timeouts, non-deterministic) — and turbo caches gate runs, so a cached re-run is a log replay, not evidence**
   Do instead: prefix every commit with `TURBO_CONCURRENCY=1 git commit …` (all ten of #29's commits needed it); re-run gates for PR-body evidence with `--force`. A hit is a log REPLAY, so "6 cached, 6 total" is evidence that inputs are unchanged, never that the checks executed here — true locally (`.turbo/cache` survives across sessions) and in CI (restored from an earlier run). The Vercel remote cache that made this cross-MACHINE is gone as of `chore/supply-chain-and-cache`; `.turbo/cache` via `actions/cache` replaced it, so nothing is shared across machines any more.
...
5. **[2026-08-13] `gh pr merge --squash --delete-branch` on a dirty worktree: the GitHub merge SUCCEEDS but the local checkout aborts and the remote branch survives**
