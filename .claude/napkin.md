...
8. **[2026-08-14] Pre-commit's full suite flakes under turbo's parallel fan-out (PGlite `beforeAll` timeouts, non-deterministic) — and turbo caches gate runs, so a cached re-run is a log replay, not evidence**
   Do instead: prefix every commit with `TURBO_CONCURRENCY=1 git commit …` (all ten of #29's commits needed it); re-run gates for PR-body evidence with `--force`. Sharper since remote cache landed (`chore/ci-minutes`): a hit can now be replayed from ANOTHER machine's run, so "6 cached, 6 total" in a local or CI log is evidence that inputs are unchanged, never that the checks executed here.
