...
| `density.mjs` | is a file within the **comment budget**? The gate — `pnpm comments` | no |
...
pnpm comments                                      # the budget, as the gate runs it
...
measurement: no `.ts`, `.tsx` or `.mjs` file we own may spend more than **3%
of its lines, or two lines, whichever is larger**, on comment-only prose.
...
reaches the budget. A **standalone** `//` is residue and does count.

Generated files (`@generated`, `GENERATED FILE`, `should not be edited`,
`Generated by` in the first eight lines) report as SKIPPED: their headers
belong to their generators, and editing one is undone on the next run.
