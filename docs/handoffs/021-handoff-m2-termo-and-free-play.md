...
**c. Accents colour shapes, not words — [#68](https://github.com/fernandolisboa/miolos/issues/68), decided 2026-08-02.** `--accent-termo` is 2.731:1 against `--paper-desk`, the worst of the four, and `--paper-card` cannot rescue it either (2.736:1). The token values **stay**; accent stops being a text colour on paper. Accent as a *fill* with a light label is already fixed (`--ink-on-accent`, 4.506:1); accent as a *non-text graphic* is unaffected. **Design Termo's screen under this rule from the start** — mustard never appears as a small label. The three shared sites still to convert (`.titleKicker`, `.barKicker`, `.chipDone .chipName`) are listed on the issue; whoever does #68 or #27 first should carry the rule into `DESIGN.md` with an ADR.
...
| # | What | Status |
|---|---|---|
| [#58](https://github.com/fernandolisboa/miolos/issues/58) | A completion synced after the São Paulo rollover derives as **late** | **Decided 2026-08-02, now `ready-for-agent`, not yet built.** The server records that a user was online on date `D`; a late sync for `D` is credited if that record exists, otherwise it lands as *played* only. Do **not** implement a grace window and do **not** widen `ACCEPTED_DAYS_BACK` (it is `1`). Amends ADR-0026 (on-time becomes decided-at-write and stored, so ADR-0009's "streak is derivable from completion rows" survives). **Barely touches #27** — Termo cannot be played offline at all |
...
| [#68](https://github.com/fernandolisboa/miolos/issues/68) | The accent palette's WCAG AA failures | **Decided 2026-08-02: accents colour shapes, not words.** Binds #27's screen design — see landmine (c) |
