...
An addendum carrying a whole separate ticket used to live below this line.
It was #104's, it corrected nothing 063 says, and it pushed this file to 240
lines — second only to handoff 058, which `CLAUDE.md` names as the
anti-example. It was moved to handoff 069 on 2026-08-22 (Fernando), and the
rule it broke is now explicit: **a session closes with its own numbered
handoff; an addendum is only for correcting what its handoff already says.**
Handoff 062 is the right shape — four addenda, 105 lines, every one an
amendment.

Addendum A stays here because it is a different session's record and is not
mine to relocate. It is the same class of misfiling: **#64 should have been
its own handoff too.**
