...
**And it has already earned its keep a second time, on this handoff's own gate.** Run [`32199901210`](https://github.com/fernandolisboa/miolos/actions/runs/32199901210) — genuine, `0 cached, 6 total`, all six `cache miss` — measured the rails at **1 042 ms**, which is *above* the 990 ms the comment calls its maximum. Under the pre-#109 reading that is a falsified record on a **docs-only PR that touched no test**. Under the clause, `1 042 < 2 000` and **nothing is owed**: the sample maximum moved, the verdict did not, and no comment, budget or range changes. The four sudoku sites on the same row are clear by a factor of four — `P1` 144 616 against the 312 500 tripwire, `P2` 56 639 against 120 000, `P3` ramp 22 613 against 50 000, full-week 7 532.
...
- **Eleven genuine CI gate rows now exist for the sudoku tests**, not the seven plan 051 §2.1 was written against — #124's branch took three ([`32195070489`](https://github.com/fernandolisboa/miolos/actions/runs/32195070489) at `0 cached, 6 total`, [`32196991090`](https://github.com/fernandolisboa/miolos/actions/runs/32196991090), [`32198441929`](https://github.com/fernandolisboa/miolos/actions/runs/32198441929)) and this handoff's own PR added [`32199901210`](https://github.com/fernandolisboa/miolos/actions/runs/32199901210). **Every docs-only PR adds a row too** — the gate runs the whole suite regardless of what changed, which is exactly why §3.3's clause has to exist. The four sudoku comments' *"seven genuine gate runs"* scoping is the **anchoring window** and stays as written: more runs existing does not falsify a statement about those seven. Rows 2 and 3 read `Cached: 1 cached, 6 total` — the single replay is `@miolos/ui:test` (1 file, 3 tests, ~1.1 s), stated rather than hidden.
