...
*(The rows below record revision 3 **as written at step 4**, including where it was wrong. Step 7 moved the drift tripwire from `budget / 4` to `budget / 2` and corrected two numbering claims; both are **deviation D15** in §14, and every live statement of the threshold elsewhere in this document reads `budget / 2`.)*
...
| binairo `P1` | `packages/games/test/binairo/generate.test.ts` `P1` (`:24-67` unedited, `:24-86` after this PR's comment) | 5 000 (default) | 3246 | **64.9 %** |
| `T-LINT-S9` | `apps/web/test/eslint-free-play-wall.test.ts` | 5 000 (default) | 2915 | **58.3 %** |
| binairo `P2` | `packages/games/test/binairo/generate.test.ts` `P2` (`:69-78` unedited, `:88-111` after this PR's comment) | 5 000 (default) | 2667 | **53.3 %** |
...
9832 × 4 = 39 328  →  40 000 ms          (drift tripwire: budget / 2 = 20 000 ms — D15)
```
...
5922 × 4 = 23 688  →  25 000 ms          (drift tripwire: budget / 2 = 12 500 ms — D15)
```
...
3. **`eslint-db-wall.test.ts` has five top-level `describe`s** — at lines 84, 630, 692, 763 and 839 in the unedited tree, **re-derived at merge time as 115, 661, 723, 794 and 870** (exit criterion 22, D15: this PR's own insertion moved every one of them); the other two files have one each — `eslint-free-play-wall.test.ts:62` → **`:89`**, `eslint-og-wall.test.ts:118` → **`:146`**. A `describe`-level option needs five edits in that file alone, seven across the trio, and a sixth `describe` added later silently drops back to 5000 ms. `vi.setConfig` has no such hole.
...
>
> If any of the five figures exceeds `budget / 2` — `40 000 / 2 = 20 000 ms` for the walls, `25 000 / 2 = 12 500 ms` for binairo — **that is a defect to diagnose and record, not a number to raise and not an automatic re-derivation** (D1's residual: the anchor is a sample maximum and it moves). **`budget / 2` and not `budget / 4`, and the derivation is D15's:** `budget = anchor × 4`, so `budget / 4` *is* the anchor and a tripwire there fires on exactly the new sample maximum D1's residual predicts as normal. Raising a shipped timeout requires fresh measured figures in the PR body and an updated arithmetic comment.
...
A header insert and an append inside a describe do not overlap textually; git resolves both automatically in either order. **The same holds for D5's rejected `describe`-options shape** (m11): it would touch the seven `describe(` headers — `eslint-db-wall.test.ts:84, 630, 692, 763, 839`, `eslint-free-play-wall.test.ts:62` and `eslint-og-wall.test.ts:118` in the unedited tree; `115, 661, 723, 794, 870`, `:89` and `:146` at merge time (D15) — header lines, still disjoint from appends inside those describes' bodies. Either shape auto-merges; the analysis does not depend on which one ships.
...
**What #106 must do:** rebase onto `main` after #107 merges, re-run its gate on the rebased tree, and — because it touches all three wall files and `eslint.config.mjs` — **discharge §5.2's re-read obligation**: put the five §2.2 tests' CI durations from its own gate run in its PR body, and treat anything over 20 000 ms (walls) or 12 500 ms (binairo) — `budget / 2`, D15 — as a figure to diagnose and record rather than a number to raise. It inherits the 40 000 ms budget for its new probes automatically, because D5 is file-scoped; a per-test decision would have required #106 to remember to add one. That is a second, smaller argument for D5.
...
**The pattern is anchored on both axes, and the loose version of it was wrong on both (R5).** Revision 3 wrote `'✓ (T-LINT-1|T-LINT-S9|T-LINT-S41|P1 —|P2 —)'`, which over-matches twice over. `T-LINT-1` also matches **`T-LINT-10`** (`apps/web/test/eslint-db-wall.test.ts`, *"the table-literal bans fire at .js, .jsx and .mjs paths too"* — `:746` in the unedited tree, **`:777`** at merge time; the id is the durable citation, D15) — a different test in the same file, silently added to the five. And `P1 —` / `P2 —` match **sudoku's** properties as well as binairo's: `packages/games/test/sudoku/generate.test.ts:49` (`P1 — determinism`), `:63` (`P1 — pinned regression`) and `:75` (`P2 — solvability`). That second one is the damaging half, because exit criterion 6 applies the binairo tripwire (`budget / 2 = 12 500 ms`, D15; `budget / 4 = 6250 ms` as this revision shipped) to whatever this grep prints, and sudoku `P1` runs at **155 025 ms against its own 240 000 ms budget** (§2.3, 64.6 %, a known #109 residual) — it would read as a catastrophic breach of a threshold that does not apply to it. The corrected pattern reads the test **ids with their trailing colon**, which `T-LINT-10:` does not satisfy, and gives the binairo properties prefixes unique to their file: `P1 — generated` (`binairo/generate.test.ts:24`) and `P2 — determinism` (`:69`) — sudoku's determinism property is `P1`, not `P2`, so neither prefix collides. All five names re-read from the tree in this session. The leading `✓ .*` matches §2.3's own sweep shape: the tick is followed by the file path, not by the id.
...
> **If any of the five exceeds `budget / 2` — 20 000 ms for the wall trio, 12 500 ms for the binairo pair — the figure is diagnosed and recorded in the PR body: which test, which run, what else was on the box, and whether it is drift or a draw. It is *not* an automatic re-derivation and it does *not* block the merge.** *(D15: the threshold shipped at `budget / 4` and was moved at step 7. `budget = anchor × 4`, so `budget / 4` is the anchor itself — a line there fires on the new sample maximum this very rule predicts as normal, and it did, four times on unchanged code.)*

**Why this is no longer a pre-merge blocker (C1, C2).** Revision 2 wrote the same rule as a merge gate at 8750 ms, and round 2's sixth run tripped it outright at 9832 ms with a second test at 97.4 % of it. That is the predictable consequence of setting a trip-wire at a **sample maximum of a distribution whose sample maximum keeps climbing**: over ten runs it is a near-certain trip, and a gate that fires on the expected behaviour of its own estimator trains the next agent to route around it. ADR-0055 decision 4 already gives the correct disposition — *"a defect to investigate"*, an obligation to diagnose and record — and §10.1 now says the same thing instead of contradicting it. **What still blocks the merge is the AC: ten green runs.** A figure over `budget / 2` alongside ten green runs is a finding to write down; a figure that goes red is a red run and fails criterion 4 on its own.

**And at `budget / 4` the threshold had no margin at all, which is what step 7 corrected (D15).** 10 000 ms against a pooled maximum of 9832 ms is 168 ms of headroom — the honest reading was that the *next* wall figure over the line was more likely than not, and it was: an independent eight-run block on this same branch, unchanged code, an agent session resident, produced **11 733 / 10 578 / 10 510 / 10 138 ms** — four crossings, ten green runs. A line that fires four times on a correct merge candidate is not measuring drift, it is measuring the estimator. `budget / 2` — 20 000 ms for the walls, 12 500 ms for binairo — is twice the anchor, a figure the pooled sample has never approached, and it still records rather than blocks.
...
- the five tests of §2.2's table with their new CI durations, checked against §10.1's `budget / 2` rule (20 000 / 12 500 ms, diagnose-and-record; D15). The timeouts make nothing faster — the expectation is that `T-LINT-1` still costs ~4 s and now has ~36 s of headroom instead of 814 ms.
...
Exit criterion 22 requires the same audit at merge time rather than trusting this one — **and step 7 widened it (D15): the audit runs over the *living* corpus too, not only over this plan.** A citation this PR makes stale is a defect this PR owns, and the instance is row 17 of §12.9: the napkin's item 3 cites a line number inside `eslint-db-wall.test.ts`, which is one of the four files this PR inserts into.
...
**Number: `0055`.** Handoff 041 §5 records *"Next ADR **0055**"* (verified). **Revision 1 asserted that `0056` is reserved for #96, revision 3 dropped that claim as unsourced (m2), and dropping it was the error — corrected at step 7 (D15).** The claim was dismissed on a `grep -rn "0056"` scoped to *this worktree*, which is the instrument failure this plan diagnoses everywhere else: #96 is a parallel ticket on its own branch, and a grep of one worktree cannot see another branch's commits. Verified against the branch instead of the tree: `git diff --name-only eea445e feat/96-archive-done-chip -- docs/` lists **`docs/adr/0056-the-record-snapshot-cache-is-per-key-and-the-done-chip-wears-the-hub-word.md`**, committed in `72b27d1`. **The reservation was real. This ticket takes 0055; the next free ADR is `0057`.** *(Root cause, recorded because it is not a reviewer's failure: the 0055/0056 split was assigned in orchestration and never written into the repo, so no reader of `main` could see it. The numbers never actually collided; only the text was wrong.)*
...
4. **A timeout is a ceiling, not a target.** A test over **`budget / 2`** is a defect to investigate and record, never a number to raise, and never an automatic re-derivation either — decision 2's residual means a single figure over the line may be a draw rather than drift, so it is diagnosed and written down before anything moves. Raising a shipped timeout requires fresh measured figures in the PR body and an updated arithmetic comment. **The threshold is derived, not picked (D15):** decision 2 sets `budget = anchor × 4`, so `budget / 4` **is** the anchor — a line there fires precisely when a session sets a new sample maximum, which decision 2 predicts as the estimator working. `budget / 2` is twice the anchor: drift rather than a draw. **The rule, general form:** *a PR that touches a timed test's cost driver re-reads that test's CI duration out of its own gate log and into its PR body*, and treats a figure over `budget / 2` as a defect to diagnose and record. The cost driver is often not the test file, so the list is written per timeout. **#107's instantiation, and it is an instantiation and not the rule:** the three ESLint wall suites; `eslint.config.mjs`; `packages/games/test/binairo/generate.test.ts`; **`packages/games/src/binairo/**`**; or the **version of `eslint`, `eslint-config-next` or `typescript-eslint`** in the root `package.json`/lockfile. The last two exist because they are where drift actually originates and neither touches the first three. **Every timeout decision 1 causes to be added later owes its own list in the PR that adds it** — starting with #109's three. #106 is the first PR to owe the re-read.
...
- **A slow drift inside the budget is invisible until it crosses `budget / 2` on a PR that happens to touch one of decision 4's five triggers.** No mechanical checker detects growth; decision 4's procedure is a human obligation, and this is the honest residual.
...
- **`044` / `045` — and *not* `043`, which is the correction this revision's own first pass needed.** Handoff 041 §5 records *"Next plan/handoff **042**"* (verified, `041-…:118`), and reading the next two free numbers straight off that line gives `043`/`044`. **`043` is not free.** #96 is a live parallel ticket running in `/home/ferna/projects/miolos-wt/96` — the worktree landmine 14 and §10.3 already tell this ticket not to touch — and its plan is `docs/plans/043-issue-96-plan-archive-done-chip.md`, uncommitted but written. A number claimed by an in-flight ticket is taken; racing it into the same sequence slot is the same defect one corpus over. **Resolution: #107 takes `044` and `045` and leaves `043` to #96.** So this PR consumes **042, 044 and 045**, and `043` belongs to #96. **What this revision then got wrong, corrected at step 7 (D15): the next free document is not `046`.** #96's branch has since committed three more documents, verified against the branch rather than against this worktree — `git diff --name-only eea445e feat/96-archive-done-chip -- docs/` returns `docs/plans/043-issue-96-plan-archive-done-chip.md`, `docs/plans/046-issue-96-research-review-round-1-findings.md`, `docs/plans/047-issue-96-research-review-round-2-findings.md` and `docs/adr/0056-…`. **The next free document is `048` and the next free ADR is `0057`.** The next handoff records those two numbers, and records the instrument as well: **a numbering claim is checked against every live branch, never against one worktree's `ls`** — a shared sequence with parallel tickets in it cannot be read from a single checkout.
...
| 14 | `docs/handoffs/041-…:118`, *"Next plan/handoff **042**. Next ADR **0055**."* | the numbering line | **Consumed**: this PR takes 042, **044**, **045** and ADR 0055; **`043`, `046`, `047` and ADR `0056` are #96's**, committed on `feat/96-archive-done-chip` (§12.5, D15). Snapshot left as written; the next handoff records **048** / **0057** |
...
| 17 | **Living records citing `file:line` inside a file this PR edits** — the class, not one instance | `.claude/napkin.md` Execution item 3 cites the `code()` stripper at `eslint-db-wall.test.ts:812-816`; it was `:888` on `main` and this PR's insertion moves it again | **Citation converted to the symbol name** (`function code(`), which does not rot, §12.1's file. **The general check this row adds:** a PR that inserts lines into a file must grep the *living* corpus (`.claude/napkin.md`, `CLAUDE.md`, `CONTEXT.md`, `docs/agents/`) for `<basename>:<line>` citations into that file and re-derive or de-line them. Snapshots (handoffs, plans) are left as written |
...
6. §10.1's rule discharged **in writing, in the PR body**: the five per-test durations from all ten Block A runs pasted, and any figure over `budget / 2` — **20 000 ms** for the wall trio, **12 500 ms** for the binairo pair (D15; the criterion shipped at `budget / 4` = 10 000 / 6250 ms) — **diagnosed and recorded** (which test, which run, what else was on the box, drift or draw). This is not a merge blocker and a figure over the line does not by itself force a re-derivation (C1/C2, ADR-0055 decision 4); ten green runs remain criterion 4's job. **What does block: a silent block A** — the durations are pasted whatever they say.
...
19. **#109 and #110 exist, are labelled `ready-for-agent`, and are cited from §1, §5 and §14** — the residuals and the cap are in the tracker, not only in this snapshot (m9). Both carry the **native GitHub dependency edge** — `gh api --method POST repos/fernandolisboa/miolos/issues/<n>/dependencies/blocked_by -F issue_id=$(gh api repos/fernandolisboa/miolos/issues/107 --jq .id)`, which is `docs/agents/issue-tracker.md:42`'s **canonical** form and the one the frontier query at `:43` actually filters on (`issue_dependencies_summary.blocked_by > 0`) — **and** the prose `Blocked by: #107` line at the top of the body, which is the fallback form and is kept as human-readable redundancy (F12, and step 7's M4: with the prose line alone, both issues surfaced as unblocked `ready-for-agent` frontier work while #107 was still open), and **#110's body carries the decision-2-rewrite obligation** as well as the ADR-0017:21 annotation and the fifth-decision obligation (F10).
...
| **D14** | §10.1's extraction pattern, `grep -hE '✓ .*(T-LINT-1:\|T-LINT-S9:\|T-LINT-S41:\|P1 — generated\|P2 — determinism)'`, and the R5 paragraph that hardened it | The same five anchors, but matched **after stripping ANSI**, and with no space required after the tick: `sed -e 's/\x1b\[[0-9;]*m//g' <log> \| grep -aE '✓.*(…)'` | R5 corrected this pattern on both of the axes it thought about — `T-LINT-1` over-matching `T-LINT-10`, and `P1 —`/`P2 —` over-matching sudoku's properties — and **both corrections are right and were load-bearing** (`T-LINT-10:` really does exist — `eslint-db-wall.test.ts`, `:773` as this row was written and **`:777`** at merge time, which is itself D15's point about line citations). It missed a third axis: the literal space in `'✓ '`. Vitest's default reporter emits the tick followed by **ANSI colour codes**, and turbo prefixes the whole line with `@miolos/web:test: `, so the rendered line is `@miolos/web:test:  <ESC>[33m<ESC>[2m✓<ESC>[22m<ESC>[39m T-LINT-1: …` — the character after `✓` is `ESC`, not a space, and the pattern matches **nothing**. Block A's first extraction returned zero lines for all ten runs against a criterion (exit 4 and 6) that treats a silent block as the one blocking failure. **No re-run was needed**: the figures were in the ten saved logs all along, so this cost an extraction pass, not a measurement block. Landmine 13 already records that `gh run view --log` needs an ANSI stripper for exactly this reason; the local reporter needs the same one and §10.1 did not say so. **Severity: low, and it is a plan defect rather than an implementation one** — recorded because the criterion it silently defeats is the one the plan calls non-negotiable |

#### Batch 2 — 2026-08-16, step 7 (fix, after step 6's three reviewers)

| # | From | To | Cause, and severity |
|---|---|---|---|
| **D15** | Three step-7 corrections, listed together because they share one cause — a claim checked against the wrong instrument. (a) §3 D2/D3, §5.2, §10.1, §10.5, §12.4 decision 4, §12.1's napkin draft, exit criterion 6 and landmine 7 all set the drift tripwire at **`budget / 4`**. (b) §12.4 dropped revision 1's *"`0056` is #96's"* as unsourced, and §12.5 handed forward *"the next free document is `046`"*. (c) §12.1's shipped napkin item 4 came out at **239 words** against F11's 179-word draft, and §12.9's forward table had no row for living records this PR makes stale | (a) the tripwire moves to **`budget / 2`** — 20 000 ms for the walls, 12 500 ms for binairo — everywhere it is stated, with the derivation written into ADR-0055 decision 4, which also states the re-read obligation as a **general rule** with #107's five paths as its instantiation. (b) `0056`, `043`, `046` and `047` are **#96's, committed** on `feat/96-archive-done-chip`; **next free document `048`, next free ADR `0057`.** (c) item 4 cut to **179 words**; §12.9 gains **row 17** and the napkin's `code()` citation is de-lined to the symbol name | **(a)** is the one that was load-bearing. `budget = anchor × 4`, so `budget / 4` **is** the anchor, and a tripwire there fires exactly when a session sets a new sample maximum — which D1's own residual paragraph predicts as normal. Decisions 2 and 4 therefore contradicted each other by construction, and the contradiction is not theoretical: an independent eight-run block on this branch, unchanged code, an agent session resident, measured `T-LINT-S9` **11 733 ms**, `T-LINT-S41` **10 578** and **10 510 ms**, `T-LINT-S9` **10 138 ms**, `T-LINT-1` **9589 ms** and binairo `P2` **6134 ms** — four crossings of the 10 000 ms line, ten green runs, nothing to diagnose. **The budgets do not move: 40 000 / 25 000 stand.** 40 000 ms is still 3.4× the largest wall figure ever measured anywhere, the crossings are the estimator behaving as D1 says it behaves, and re-deriving off a noisier session would be the sizing-by-latest-sample error §12.7 and landmine 7 both forbid. **(b)** was dismissed on a `grep -rn "0056"` scoped to **this worktree**, which cannot see a parallel branch's commits — the instrument failure this plan diagnoses in four other places, committed by the plan itself. The numbers never collided; only the text was wrong, because the orchestration-time split was never written into the repo. **Severity: (a) medium** — it ships a rule that fires on correct work, which is how a tripwire gets routed around; **(b) medium** — a wrong number handed to the next session is a collision waiting to happen; **(c) low** |
