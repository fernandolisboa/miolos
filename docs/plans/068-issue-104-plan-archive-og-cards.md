...
Rows 1–8 came out of step 5: six from the measured spike (`~/miolos-session/104-step5-spike.md` §11) and its sizing harness, and two — rows 5 and 6 — from re-reading the plan's own citations off disk. Listed here so the step-6 reviewers can check each against its source rather than re-deriving it.

**Rows 9 onward were added at step 7**, from the implementation's own measurement and from the six step-6 lenses. Row 9 is the one that matters: a single unmeasured sentence in §2.2 propagated into six records before the implementer measured it false. The body is not rewritten (`docs/README.md:22`); each row names where the correction actually lives.
...
| 9 | §2.2's and §2.3's *"a malformed `[data]`/`[mes]` segment … now inherits the **archive** card by nearest ancestor"*. **FALSE, twice over.** Such a segment answers 404 through `notFound()`, and Next 16.2.12 **discards** the route's composed metadata on a not-found (`resolve-metadata.js`'s `collectMetadata`: once an error convention is set the metadata module read is the **layout**, never the page) — so the head carries the ROOT card. And an `opengraph-image` file reaches descendants only from a segment owning a `layout.tsx`, which `app/arquivo/` does not: `find apps/web/app -name layout.tsx` returns exactly one file, the root. ADR-0054 D8's residual is **UNCHANGED, not narrowed**, and its word *"every"* was never within #104's reach — it is about the four archive **play** routes | §2.2, §2.3, §11.2 rows `:418-420` and `:521-531`; ADR-0071 decision 2 and consequence (d); the ADR-0054 header, its inheritance-block annotation and its D8 annotation; `docs/README.md`'s ADR-0071 row |
| 10 | §4.2's **case A does not exist.** A well-formed date or month the wall refuses also `notFound()`s (`[data]/page.tsx:139`, `mes/[mes]/page.tsx:91`), so by the same mechanism as row 9 its head advertises the ROOT card, which resolves 200 — exactly as before #104. Only case B (today's `redirect()` 307, which is not a `notFound()`) is a real residual | §4.2, §16 D7, ADR-0071 consequence (c) |
| 11 | §2.2's *"unchanged"* / the spike's *"not 'roughly': byte-for-byte"*. **Unchanged to 0.1 MB, not to the byte.** Measured on two full builds at step 6: `/arquivo` **+1,221 B**, `/arquivo/[data]` **+4,065 B**, `/arquivo/mes/[mes]` **+4,069 B** — **≤ 0.15 %**, caused by the `.alt.txt` metadata module (207 B) entering every `app/arquivo/**` graph plus i18n chunk churn. Substance untouched: 2.7 MB against 23.6 MB is the load-bearing distinction. Also §2.2's *"the total is smaller, not merely better distributed"* is true of the **summed per-function** trace (556.1 MB over 43 entries shipped, against 508.5 over 40) and false of **unique deployed bytes**, which grew only **+111,441 B** | ADR-0071 decision 3, the ADR-0054 D9 checked annotation, `docs/README.md`'s two #104 rows |
| 12 | §13's *"≈ **58 s** per full sweep"*. **Rasterisation only, and derived off the wrong card.** Re-derived off `archiveCard`'s own measured p50s (day **45.9 ms**, month **51.3 ms**, both cheaper than `gameCard`'s 49.7 ms) that is ≈52 s — but Vercel bills wall-clock, so the **+1,132 Neon round trips the same paragraph counts are billed too**: **≈75–109 s** at a 20–50 ms round trip, unmeasured. And **+25.8 % is a floor**: it assumes four games a day, and on ADR-0053 D3's ragged days it is **+33 % to +100 %** | §13, ADR-0071 consequence (e), the ADR-0054 D9 denial-of-wallet annotation |
| 13 | §13 sizes a **crawler sweep** and reads as though the URL count bounds the bill. It does not: with `no-store`, no `middleware.ts`, no rate limit and no `vercel.json` firewall rule, one attacker on one URL costs N round trips and N rasterisations for N requests, unbounded. Pre-existing across the eight existing card routes; #104 widens it 25.8 % and fixes none of it | ADR-0071 consequence (e), the ADR-0054 denial-of-wallet consequence annotation |
| 14 | Two DB facts the plan asserted without an engine. The month bound rides the **composite** PK `daily_puzzles_game_date_pk (game, date)`; there is no `date`-only index. The step-6 `EXPLAIN` ran on **PGlite (PG 18.3)**, whose skip scan production does not have — **Neon is PG 17** (plan 009 D1) and falls back to a full scan of a table measured at **2,048,000 B / 4,384 rows** at three years, still sub-millisecond. **No index is added at #104** (one buys 0.2 ms). Separately: `no-store` on the **200** arm buys ~nothing for *these two* cards — they carry a date and `messages` constants and zero puzzle content, so `killed_at` only flips 200 ↔ 404, a fact `/arquivo` publishes anyway. The **404** arm's justification (K2) stands | ADR-0071 consequence (e) |
| 15 | §5's *"Four strings"*. **Five.** Rung 2 (row 8) minted `archiveDayCaption` for the year the display line dropped, after §5 was written | ADR-0071 consequence (h), plan 040's `:1092` row, `src/og/copy.ts` |
| 16 | §4.1's *"a satori throw must be a 500"*. **True of the BUILDER, not of satori.** `next/dist/server/og/image-response.js` commits `super(readable, {headers, status})` before the stream's `async start()` runs satori, so a satori throw truncates a 200. `archiveCard()` is synchronous and called before `new ImageResponse(...)`, so `T-WEB-S334` row (9) is honest about what it tests; the sentence generalised past it. Pre-existing on all ten card routes | ADR-0071 decision 4, `og-image.node.test.ts` row (9) |
| 17 | §12.3's *"already rendered and evidenced at #34"*. There is **no `docs/evidence/34-*` directory** — #34 predates the per-issue evidence convention. The "before" is the committed `app/opengraph-image.png` itself, added by #34 and byte-unchanged on this branch, and the evidence README SHA-pins that path instead of re-rendering it | `docs/evidence/104-archive-og-cards/README.md` |
