...
## Step-7 addendum (2026-08-01, append-only — the sections above are the unmodified snapshot)

- The step-5 amendment's "follow-up issue filed from #39" is **issue #51** ("chore: request/track impeccable value-level ignores for low-contrast and cream-palette").
- §9's "Not touched: `pnpm-workspace.yaml` (allowBuilds unchanged, deliberate)" did not survive implementation: pnpm 11 forced an explicit `allowBuilds` entry during `pnpm add`, so the shipped diff adds `puppeteer: false` plus a comment. Intent preserved — the plan's "deliberately do NOT allow the puppeteer postinstall" posture is now an explicit deny instead of an implicit one; Chrome still comes only from the explicit install step. Disclosed in the PR #52 body.
- Step-6 review added a preflight step to the workflow (assert non-empty `target_url` and bypass secret, allowlist the host to `*.vercel.app` before appending the secret, and require HTTP 200 on the deployment host through the bypass) — closing the proven silent-green paths where `impeccable detect` exits 0 on an empty/unreachable URL, or scans the Vercel SSO page after a secret rotation.
