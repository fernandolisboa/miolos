...
| T-WEB-S43 rails | 5 000 default | 990 (32196991090, this PR's own gate) | 19.8 % | <L5> | measured-and-fine, no timeout | 5 000 default |
...
- This PR's gate on the merge candidate `4127ad9`: **run `32196991090`**, conclusion `success` (gate 6m16s, detect 2m0s, three Vercel checks pass; zero `Test timed out` / `Hook timed out` / ` FAIL ` in the log). `@miolos/games:test` **`cache miss, executing c63d52e6b141e66e`** and `@miolos/web:test` **`cache miss, executing 8c12ece769dba0cf`** — both packages carrying #109's tests executed, so both figure sets are eligible. `nproc` = 2, `free -m` total 7 938 MB. `generate.test.ts (11 tests) 166670ms`; P1 **104 639 ms** (< 312 500, 16.7 % of 625 000), P2 39 757, P3 ramp **16 374 ms** (< 50 000, 16.4 % of 100 000), full-week 5 783; `nonogram-screen.test.tsx (58 tests) 11254ms`, rails **990 ms**. `Tasks: 6 successful, 6 total`; `Time: 4m2.464s`.
- **Caveat on the cache line, stated rather than hidden: `Cached: 1 cached, 6 total`, not the `0 cached` every earlier row in plan 051 §2.1 shows.** The single replay is `@miolos/ui:test` (1 file, 3 tests, 1.10 s) — the smallest suite in the repo, so its absence from the concurrent load is a negligible change to the contention basis. The two packages whose figures this PR uses both executed, which is the condition §6.4 actually requires.
- The PR's **first** gate, on the superseded commit `e021be4`, is also a genuine row (run `32195070489`, `0 cached, 6 total`, `Time: 4m13.03s`): P1 106 015 · P2 38 305 · P3 ramp 17 602 · full-week 5 768 · rails 835 ms. So the branch contributed **two** genuine rows, not one, and the pooled CI population is nine runs, not eight (plan 051 §12 Step-7b).
...
8. **Read the gate log** per §6.4; edit the PR body with the merge candidate's row and the cache-miss lines (run `32196991090` on `4127ad9` — the figures and the `Cached: 1 cached, 6 total` caveat are in §8's Evidence block, already read; re-read them from the log if the branch takes another commit). Post the verdict comment on #109 (§7 R5) **before merge, never after** (`Closes #109` auto-closes on squash — handoff 048 landmine 4). That comment carries, beyond the verdict table and the gate-row figures:
   - the **2 368 / 1 959 ms re-label** in the code comment's own vocabulary — *contended local, uncapped, `apps/web` at pre-#120 `maxWorkers = 7`; superseded by the re-take at the shipped bound (624 ms pooled max over 3 runs), CI 990 ms max over 9 genuine gate runs — the new maximum set by this PR's own gate, `32196991090`* — because the row in the issue body is Fernando's and stays as written (step-6 issue F2);
...
- [ ] #109 has the verdict comment **posted before merge**, carrying the verdict table, the gate-row figures (`32196991090`), the 2 368 / 1 959 re-label, the `grep -c timeout` = 1 note and the "measured, nothing owed, not filed" line (§10 step 8); PR merged; issue closed.
...
- **A1** — applied: both leaked `(load-bearing)` plan tags deleted from shipped comment text; the header's became prose — the shipped words are *"— the finding everything below rests on:"*, which drops the marker idiom rather than re-importing the word — and the `P1` site's is gone entirely.
...
#### Step-7b changes (2026-08-18, the verification round's one fix and its nits)

The step-7 verification round returned **FIXES NEEDED — one item**, and it did not come from step 7: it came from the PR's own gate. Every step-6 disposition this register claims was re-checked in the tree and found present.

**FIX-1 — the rails anchor is superseded by this PR's own gate, so the comment is re-read rather than left standing.** Run `32196991090` on the merge candidate `4127ad9` measured `T-WEB-S43` at **990 ms**, above the 944 ms the shipped comment called the CI maximum and outside its own *"376–932 ms on the others"* range. `apps/web/test/nonogram-screen.test.tsx`'s comment now reads **990 ms over nine genuine gate runs (`32196991090`; 376–944 ms on the other eight) = 19.8 % of the 5000 ms default**. **Nothing else moves, and that is the point:** 990 is far under decision 1's 2 000 ms trigger, and `990 × 4 = 3 960 → 5 000` is the default the `it` already rides, so the *measured-and-fine, no timeout* verdict and every literal in the file are unchanged. §8's verdict row and §10 step 8's pre-written re-label sentence (which said `835 ms`, the previous gate's figure) are re-read to match.

**And the clause that stops this from recurring every gate.** The comment now states, in ADR-0055 decision 2's own terms, that **the anchor is a SAMPLE maximum**: a later run above it is the estimator working, not a falsified record — decision 2's stated residual, and the reason `budget / 2` and not `budget / 4` is the tripwire. So the comment owes an update only when a sample crosses the **2 000 ms trigger**, never on every new gate row. Without that clause this ticket ships a record that goes stale on the next push, which is the class of defect #109 exists to close.

**The population is nine runs, not eight — §6.4 under-counts by one.** §6.4 calls the PR's own gate *"the eighth genuine CI row"*, written when the branch was expected to take one gate. It took two: `32195070489` on `e021be4` (`0 cached, 6 total`, all six `cache miss`, `Time: 4m13.03s`) is as genuine as any row in §2.1, so the merge candidate's is the **ninth**. Both rows are in §8's Evidence. §6.4 is left as written — a plan is a snapshot — and this is where the two disagree.

**The other four sites were re-read against both new rows and none of them is false.** `P1` 104 639 / 106 015 inside the quoted *"85 994–147 606"*; `P2` 39 757 / 38 305 inside *"32 122–61 783"*; `P3` ramp 16 374 / 17 602 inside *"14 144–24 114"*; full-week 5 783 / 5 768 inside *"4 391–8 072"*. Every new figure is below its site's pooled maximum, so **no range was widened, no anchor moved and no budget changed** — `625_000 / 240_000 / 100_000 / 60_000` and `numRuns` 100 / 100 / 35 are byte-identical to step 7's. The four comments' *"seven genuine gate runs"* scoping is the anchoring window of §2.1 and stays as written; more runs existing does not falsify a statement about those seven.

**The nits, one line each.**

- **N-1** — applied: this register's **A1** entry quoted replacement text (*"— the load-bearing finding:"*) that never shipped; it now quotes the tree (*"— the finding everything below rests on:"*). A record quoting text that does not exist is this ticket's own thesis, so it is fixed here rather than noted.
- **N-2** — applied: `generate.test.ts`'s ragged short line (*"// comment claimed. So this is decision"*) is reflowed with the four lines after it. Pre-dated step 7; A2/A4's *"both blocks reflowed"* is now true of the `P1` block too.
- **N-3** — **no edit, with the reason.** `4.4×` and the `#123` pointer appear in both ADR-0055 (l) and (n) and the reviewer marked it *"reads coherently either way"*. They do different work: (l) **records** the drift with the two run ids that bracket it, (n) **uses** it to argue that drift on the gate is environmental and needs no trigger touched, and defers the ids to (l) explicitly. Removing either leaves one of the two arguments citing nothing.
- **N-4** — applied: `.claude/napkin.md` #4 cited *"ADR-0055 (n)"* for the `35 s → 155 s` pair, which A5 moved to **(l)**. Citation corrected; the pair is stated inline in the napkin, so nothing was broken, only one hop short.
- **N-5** — applied as part of FIX-1: §10 step 8's pre-written `835 ms` was the gate on `e021be4`, not the merge candidate's. It now carries `990 ms` and the run id, and §8's Evidence block carries the whole row so step 8 pastes rather than re-derives.
- **O-1 / O-2** — info, no edit owed. O-2 (the new `domain.md` sentence's *"a new rule of any weight is an ADR"* proviso pointing against #109's own (o)) is already §8 "Decisions for Fernando" item 3; step 8 names the tension in one clause on the PR body rather than leaving a reader to find it.
