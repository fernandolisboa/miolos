...
- **A1 (plan-doc number 013 vs next-free) — APPLIED:** renumbered to 008 (verified next-free: docs currently use 001–007) with an explicit re-check-at-merge note; §5 and commit 4 updated.
  - _Amendment (step 7, PR #44):_ superseded by the orchestrator's later reservation recorded in this document's header — six parallel streams hold 008=#41 … 013=#26, and 008 landed via PR #42. The committed filename `013` follows the header; this bullet is kept as-written per the append-don't-edit rule for point-in-time docs.
