...
**Deviations appended at step 7 (review fixes, 2026-08-13):**

7. **D6/§4.2 (the clamp — corrected at step 6, F3/A-1).** The plan's clamp — `max(min(since, earliest row date), since − acceptedDaysBack)` — extends the range for ANY in-bound row, including one that colours no day. A lost row at `since − 1` (a 00:20 account losing yesterday's Termo — reachable today) or any far pre-range row would extend the range to a day no row colours, painting `"missed"` on a day the account did not exist for — contradicting D2's own guarantee. The shipped rule is **won-only**: `effectiveSince = min(since, earliest won-row date on or after since − acceptedDaysBack)`, the empty set answering `since`. A lost or out-of-bound row never extends; every extension day is anchored on a won row within the bound; aggregates are untouched (out-of-range rows still feed them). D6's text above is the plan-time snapshot and is NOT rewritten; ADR-0051 decision 2 carries the corrected rule as the record. T-CORE-S63 updated, S63a added; T-CORE-S64 asserts equality against the recomputed rule; T-API-S90a pins the lost-row non-extension at the seam.

8. **§1 "Not touched at all" (`day-state.ts`) — comment-only touches (F17).** `apps/web/src/play/day-state.ts:173` and `apps/web/test/day-state.test.ts:187` carried future-tense "#29 lands `em 4/6`" comments that the shipped `TermoDoneLink` made stale. Both reworded to the shipped state (the hub captions the tile from the server value; `DayEntry` still publishes no duration). Zero code lines changed; `DayEntry` gained nothing.

9. **§6.5/F5's histogram kicker ("Seus tempos no Binairo") — omitted, registered (F25/I-5).** The F5 frame shows a kicker line above the histogram; neither shipped surface renders it. Deliberate: both surfaces already sit under an explicit per-game heading — the conclusion under its game's own screen, the stats block under its game's card title — so the kicker would duplicate the heading directly above it. Fernando may request it in the PR.

10. **§1 (`app/page.tsx` touched beyond links) — the `todaySaoPauloDate` hoist (F7/Q-1).** The plan listed `page.tsx` as touched only for the nav links, but step 5 left `todaySaoPauloDate` duplicated verbatim between `page.tsx` and `stats-view.tsx`. Step 7 hoisted the one spelling to `apps/web/src/i18n/sao-paulo-day.ts` (exported via the i18n barrel), carrying the parts-assembly comment and the device-clock boundary (ADR-0051's consequence); both callers import it. A second touched-line class on a listed file, recorded here.
