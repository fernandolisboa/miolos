...
**Implementation deviations — Batch D, 2026-08-15 (step 5).**

| # | From | To | Cause, and severity |
|---|---|---|---|
| **D5** | §9 `S204`: the module-graph negative is *"no node's source contains `@miolos/db` or `"../src/db"`"*, cited to the shipped `expect(readFileSync(path,"utf8")).not.toContain("readDayState")` idiom | the same scan, over the file with its **comments stripped** — the `code()` helper `eslint-db-wall.test.ts:812-816` already defines for exactly this | **Medium — the mechanism as written is RED on a clean tree, so it could not have shipped.** `apps/web/src/i18n/sao-paulo-day.ts:33` says *"Declared here rather than imported from `@miolos/db`"* in a doc block and imports nothing of the kind; the root card's own doc block explains at length why it carries no `force-dynamic`. The unstripped grep reds on both, and the cheapest fix an implementer reaches for — deleting the sentences — would delete the two comments that explain the property. The stripper is the repo's own answer to this and `T-LINT-S37` already applies it for the identical reason (*"counts CODE references to `@miolos/db` and not the doc blocks that discuss the wall at length"*), so this is the plan citing the idiom one clause short rather than a new decision. `T-WEB-S200`'s `not.toMatch(/\b153\b/)` needed it too, for the doc block that states the derivation. |
| **D6** | §9 `S203`'s sentinel arm: *"by the same access, the `longDate` in that tree is `formatLongDate(row.date)` for a row carrying a distinctive published date, which is what pins the daily card's date to the DB clock's own row"* | asserted **per family, against each family's own date source**: the daily card's `longDate` moves with `row.date`, the ARCHIVE card's moves with the URL **segment** | **Medium — the sentence is false of four of the eight routes, and the plan's own §7.3 says why.** `archiveCardHandler` composes `formatLongDate(date)` from the parsed segment and never reads `daily.date` — deliberately, since the row is an existence proof there. So a floor written as *"change `row.date`, the bytes move"* is **vacuous on the archive family**: it passed on the daily four and failed on the archive four in the first green run, which is how it was found. The row now carries a `callDated` arm per family. The claim S203 makes is unweakened and is arguably sharper: each family's date is pinned to the source §7.3 specifies for it, and `T-WEB-S201`'s *"the only property read off the reader's return value is `.date`"* covers the daily half from the source side. |
| **D7** | §9's *"Mocking patterns"* paragraph, which lists `vi.mock("../src/db")` + `vi.mock("@miolos/db")` + `vi.mock("next/navigation")` as the shape | `og-image.node.test.ts` additionally **spies `gameCard` THROUGH** `vi.mock("../src/og/card", importOriginal)` | **Low, and it is `S203` row (6) that requires it.** Row (6) asserts that a throw from the card builder PROPAGATES rather than being converted to a 404 — the assertion that pins the `try` to the read alone. The obvious mechanism, a throwing field on the mocked row, provokes nothing on the archive family for D6's reason: that handler never touches the row. Spying the builder through — real implementation by default, throwing only in row (6) — is a genuine builder failure on both families, and it leaves every other row rendering the real tree. |
| **D8** | §9: **`T-LINT-S45`** is *"reserved headroom — burned if unspent"* | **spent**, on the OG surface's legal-imports control | **Low, bookkeeping — I40's accounting must not record it as burned.** The four probes above it are all bans; a wall that reds on everything proves nothing about what it bans, and `eslint-free-play-wall.test.ts`'s `T-LINT-S18` (*"the free-play directory's LEGAL surface lints clean — the wall is not a blanket ban"*) is the precedent for giving that control its own id in these files. Consequence, stated rather than left for a reviewer to hit: **the `T-LINT-*` range now has no headroom**, so a step-6 round that needs a lint id takes `T-LINT-S46`. The `T-WEB` headroom (`S210`/`S211`) is untouched and both are still unspent. |
| **D9** | §9 `S199`: *"`type` is `"website"`"*, alongside `openGraph.locale` and `siteName`, as three reads off the leaf's declared `openGraph` | `locale` and `siteName` are read directly; **`type` is asserted with `toMatchObject({ type: "website" })`** | **Low, mechanism only — the literal form does not COMPILE.** Next's `OpenGraph` type is a discriminated union (`OpenGraphWebsite | OpenGraphArticle | …`) and `type` is its discriminant, so `metadata.openGraph?.type` is `error TS2339: Property 'type' does not exist on type 'OpenGraph'` on the un-narrowed union — pasted at the batch gate before the fix. The two alternatives are both worse: a cast would be an `any`/assertion in a test whose entire subject is a declared object's shape, and narrowing by hand would have the test assert the discriminant it just assumed. `toMatchObject` reads the property structurally, with no cast and no narrowing, and `S198`'s `toMatchObject(OG_DEFAULTS)` already asserts the same member on all four dailies by the same mechanism. The claim is unchanged. |
| **D10** | §14 D2's parenthetical measurement: *"2 findings with the share block and 2 without at 390×844, **0 and 0 at 1440×900**"* | 390×844 reproduces exactly — 2 and 2 — while **1440×900 is 1 and 1**, the extra finding being `cream-palette` | **Low, and the CONCLUSION is unchanged: #34 adds zero findings at either viewport.** I34's harness inlines `packages/ui/tokens.css`, **`app/globals.css`**, `src/play/screen.module.css` and `src/play/conclusion-view.module.css`; `globals.css:12` is what paints `body { background: var(--paper-desk) }`, so a harness without it renders on browser white and `cream-palette` cannot fire. Including it is the more faithful page, so the figure is corrected rather than reproduced. `cream-palette` is a recorded by-design finding (`.impeccable/config.json`, *"the cream page background is the recorded Ateliê direction"*), and its ignore is **file-scoped to `localhost`/`vercel.app`** — which the plan forbids widening to a `file://` path, so it is reported here rather than silenced. Two other rules fire only when the run is launched from OUTSIDE the repo: `overused-font` for both faces (value-level ignores, not file-scoped, so they resolve as soon as `npx impeccable detect` runs from the repo root) and `single-font` when the harness omits the `--font-*` variables that `next/font` supplies in production — the harness declares both families for that reason. The share-block delta was measured the same way D2 measured it: the same HTML with the `.shareBlock` subtree removed reports the identical set at both viewports. |
