...
3. **Frontier narrative (§7/§10/§12):** #158 (PR #166, commit 80f030a) merged and already reconciled `docs/agents/test-ids.md` — the "correct the stale S290 row" duty would now write a false record and is dropped; the S297–S298 reservation stands; re-derive by grep (and the 063/0068 doc numbers) at commit time.
...
Green gate with pasted evidence (§8); all §7 tests landed under their reserved ids, reservation reconciled in `docs/agents/test-ids.md` against a fresh grep (§7); ADR-0068 Accepted in the same diff that ships its code; ADR-0064's three annotations committed; the §10 sweep clean; `docs/README.md` rows for 063 and 0068; the #36 comment posted (§4.7); PR merged, #146 closed. Process-weight check (ADR-0058 rule 2): the code diff here (migration, reader, dispatcher, route, workflow, ~13 test blocks) comfortably outweighs the records — the tier holds.

## 13. Step-7 deviations (2026-08-20, PR #171 review round)

Appended per §10's own last bullet; the sections above are a snapshot and stay as written.

1. **Migration 0011 exists; §6.1's "Rides `completions_user_date_idx`" was wrong for the CTE** (step-6 performance lens). The counted-day CTE filters by date range with no user predicate, and both existing indexes lead with `user_id`, so the hourly tick full-scanned `completions`. Shipped: the partial `completions_counted_date_idx` — `(date) WHERE outcome = 'won' AND on_time` — chosen by EXPLAIN measurement (a PGlite probe over the real migrations, ~26k seeded rows: the planner takes `(date)` for the range scan and refuses `(date, user_id)`, which only duplicates `completions_user_date_idx` on the point lookups). The `exists` probes do ride `completions_user_date_idx`, as §6.1 said. Applied to Neon before the branch push carrying it, per §3's ritual.
2. **ADR-0064 carries FOUR annotations, not §4.13's three** (step-6 ADR lens): annotation (d) records consequence (a) as discharged — VAPID activated 2026-08-20, dispatcher live at merge.
3. **The ADR is 0068, not 0067.** PR #170 (Termo accent) merged holding ADR-0067 first; this branch's ADR renumbered before its own merge — the unlanded-number-moves rule, every citation swept.
