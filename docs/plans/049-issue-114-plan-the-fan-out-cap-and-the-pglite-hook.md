...
**A second CI reading landed with this addendum, and it supersedes §3 D5's arithmetic.** Gate run **32081091260** reported all 26 boots at a pooled maximum of **9 733.8 ms** (32.4 % of budget), against run 32003396086's 8 686.4 ms. It is a genuine execution, not a replay: the only `cache hit, replaying logs` in that step is `@miolos/ui:test`, hash-matched against the local run, and every `t=` epoch stamp equals its own log line's timestamp. Decision 2 takes the highest, so the CI half now yields `9 733.8 × 4 = 38 935.2 → **40 000 ms**` rather than D5's stated 35 000 — **which is the same number the local half yields**, so both halves agree. §3 D5 is left as written, per this file's snapshot class; ADR-0057 and the comment at `createTestDb` carry the corrected figure.

**And the second reading measures a premise the first could only assume.** All 26 boots report `pool=1`, so vitest genuinely collapses to one worker on the 2-vCPU runner — which is exactly what ADR-0057 decision 3's CI exemption rests on, and it had never been observed directly before.
