...
Frontier as of #146 (plan 063 — its paragraph is below; the table sits at its step-5 spends). The narrative that follows is #145's (plan 061), the last full reconciliation before it: re-derived by grep over the branch at its step-5 build, again after the pre-gate `git merge origin/main` that brought #140 (`#152` — its whole reservation, `T-WEB S286–S287` + `T-API S141`, landed **burned**, per its row below), and once more after the step-7 `git merge origin/main` that brought #142 (`#153`, plan 060 — its ranges are now landed and in the table). **The 2026-08-20 parallel night's last live reservation — #58 (`T-CORE S105–S106`, `T-DB S71–S75`, `T-API S137–S140`, `T-WEB S283–S285`) — is now landed and in the table** (its paragraph below), alongside #140's burned trio and #142's burned `T-API S134…S136`, which are never reused either. It is a snapshot, not a guarantee: re-run the grep before allocating, and re-derive it at step 8 of any ticket that adds ids. The live series is `S` everywhere; the bare series are closed and nothing is ever added to them.
...
hold a record renders no share control at all, on Termo's daily conclusion and
on the archive late-result panel for all four games. Re-derived by grep before
allocating, which agreed with this table in both columns (next free `S358`,
highest in use `S357`). One id over eight cases, the `T-LINT-S49` shape.

Every `toBeNull` in it is falsifiable, and each was redded by a named mutation
rather than argued: the read probe restored reds cases 1 and 2, a token probe
value reds case 3, and case 4 — the same store given room for a record —
proves all five surfaces bring the button back. The sizing case binds
`WRITE_PROBE_BYTES` on both sides against the largest record the four schemas
admit, so a probe too small to stand for a record and one so large it hides a
control that would have worked both fail.
...
#149 (Tier 1, no plan) reserved **`T-WEB-S339…S343`** — four planned plus one
tail of review-round headroom, no `T-CORE`, `T-DB`, `T-API` or `T-LINT` id
([the reservation](https://github.com/fernandolisboa/miolos/issues/149#issuecomment-5379014119),
posted on the issue before step 5, with the frontier re-derived by the
documented grep, which agreed with this table in both columns:
`T-WEB` next free `S339`, highest in use `S336`). It spent
**`T-WEB-S339…S342`** in the new `apps/web/test/mount-mint-order.test.tsx` —
one suite per hook proving that `useAttachState`, `useStreak`, `useMedals`
and the `useStats`/`useStatsCalendar` pair issue no read until
`ensureSession()` resolves. `S342` deliberately carries **two hooks in one
claim**: `use-stats-calendar.ts` is `use-stats.ts`'s twin over the same
client module — since #206 cluster 3 they are literally the same three-line
delegation over `useMountFetch` — and the single correctness
reviewer's mutations red it through either half.

The tail **`T-WEB-S343` went unspent and is burned** in the table below. It
was live headroom through the review round and the round did not need it: the
reviewer's five mutations found two gaps, and **both are unbindable rather
than untested** — nothing observable distinguishes a hook that drops its
`cancelled` flag under React 18+, and the `.catch` arms are unreachable
through the shipped clients, which are total. Neither is a claim a new id
...
`T-WEB-S350` were the reserved review-round headroom and went **unspent**:
#195 landed at PR #203 (`6b963d9`) without either, so both are **burned** and
...
package-scoped control. The tail **`T-LINT-S60` went unspent and is burned**
in the table below: the correctness round returned no BLOCKER, and both edits
it required were comment corrections, which carry no claim and take no id.
...
live reservations while #163 and #33 were open and are **burned as of
their step 8** (both PRs merged 2026-08-21, `d10e33d` and `651ec33`;
rows in the burned table below): #163's remaining tail
`T-WEB-S313`/`S314`, and what was left of #33's own reserved
review-round headroom — **`T-CORE-S111`, and only that**:
#33's step-7 round spent `T-API-S175`, `T-WEB-S322` and `T-LINT-S53`
(the #33 paragraph below), so the highest-in-use column moved with them
while "next free" did not, because headroom is exactly what those columns
were already jumping over.

**#33's ranges are landed, not reserved, as of this merge** — the last
on-issue reservation of the 2026-08-21 parallel night to resolve. The
`T-WEB` maximum is now #33's `S322`; it was #163's `S312` (spent at its
step 7 — its paragraph below) on the revision this one replaces, and
#169's `S307` before that (`S308` burned, its paragraph and the burned
table). This branch's own pre-gate `git merge origin/main` conflicted in
this table — as #146's did, and #169's, and #163's — and was resolved the
same way every time: **keep BOTH allocation paragraphs, set the rows to the
true maxima, renumber nothing that landed** (the #103/#67 precedent —
nothing burned, each number keeps exactly one meaning). #146's own `S297`
sitting inside its reservation below the sibling ranges is the shape
...
#126 (plan 055, ADR-0059) spends **`T-WEB-S233`** on the scan over `.github/workflows/properties.yml` in `apps/web/test/fanout-cap.test.ts` (`describe("the nightly full property proof cannot be deleted in silence (T-WEB-S233)")`), reserved here in the same pull request that spends it — which is what `T-WEB-S229` did not get. **It was written as `S230` and renumbered three times before it landed — to `S231`, to `S232`, and finally to `S233`**: #63 (`#130`) took `S230` for `aligning-numerals.test.ts` while #126 was in review, and the two branches collided in this very file. Nothing is burned — `S230` is in use, by #63 — and the collision is recorded rather than smoothed over, because it is the exact failure the frontier row exists to prevent and the row did not prevent it: **both tickets read `S230` as next free, hours apart, and neither could see the other's branch.** The cheap mitigation, for the next parallel session: reserve the id on `main` (or in the issue) before step 5, not in the pull request that spends it. **One id, no tail reserved**, deliberately: plan 049's and plan 040's reserved review-round headroom was unspent both times and is burned below, so this ticket reserves the id it uses and nothing else. Nothing is burned. The assertions #126 added to `T-WEB-S227`'s `describe` — that `turbo.json` declares `MIOLOS_FULL_PROPERTIES` on the `test` task's `env` — correctly took **no** new id: same claim about the same file as `T-WEB-S227` itself, the `T-WEB-S100` burn precedent. **`packages/games` still carries no ids**, so the new `packages/games/test/property-runs.ts` and the three tests added to `packages/games/test/sudoku/generate.test.ts` take none — the #109 and #114 precedent, unchanged. **The collision happened three times in one night.** #103 (`#136`) took `S231` for `archive-share.test.tsx` while this branch was queued behind GitHub's runner backlog, and this file conflicted again. Then #67 (`#133`) took `S232` while this branch was still queued. Four tickets read the frontier row hours apart and all four were right when they read it. That is the row's structural limit, not an agent's mistake: it records what has landed, and parallel branches are by definition what has not. The mitigation stands and is now twice-earned — **reserve the id on `main` before step 5, not in the pull request that spends it.**

#158 (Tier 2 at reduced ceremony — the #103 register; the deviation and its reason are in the PR body) spent **`T-WEB-S293`** on `apps/web/test/termos.test.tsx` (the `/termos` page renders statically with its marker and every string from the messages module — the `T-WEB-S141` shape applied to the sibling page) and **`T-WEB-S294`** on `apps/web/test/hub-attach.test.tsx` (the terms link rides `routes` at both legal link sites — the `T-WEB-S143` shape). The ids came from an **on-issue reservation** (`T-WEB-S293…S296`, reserved on issue #158 before step 5 — the mitigation the three-collision night above asked for, used here); the tails **`T-WEB-S295`/`S296` were the reserved review-round headroom, went unspent, and are burned below** per the rule. The `/termos` row added to `T-WEB-S56`'s ROUTES table and the `/termos` entry in `T-WEB-S174`'s sitemap expectation correctly take no new id — the `T-WEB-S100` burn precedent. This edit also corrects the frontier row above, which still read `S290` as next free while **`T-WEB-S290` was already in use** (`apps/web/test/binairo-page.test.tsx`, the least-privilege credential scan, landed without a frontier update — the `T-WEB-S229`/#120 case again); the reservation started at `S293` precisely so the row's staleness could not collide with it, and `S291`/`S292` are a **live sibling on-issue reservation** (issue #160), jumped over here exactly as #143 and #141 jumped over #35's range — whichever of the two tickets lands second reconciles this paragraph.

#162 (Tier 2 at reduced ceremony — the #103 register; the deviation and its reason are in the PR body) spent **`T-WEB-S303`** on `apps/web/test/hub-onboarding.test.tsx` (the first-visit card is a full-width band, not a half-width note: no width cap on the desktop `.intro`, the two-column composition, the mobile return to block flow, and one static rotation angle at every site). The id came from an **on-issue reservation** (`T-WEB-S303…S306`, reserved on issue #162 before step 5); the tails **`T-WEB-S304…S306` were the reserved review-round headroom, went unspent, and are burned below** per the rule. `T-WEB-S253`'s reduce-block expectation was updated in place for the one angle (`rotate(-0.5deg)` → `rotate(-0.3deg)`) and correctly took no new id — the same claim about the same block. When it landed the row above jumped over two live sibling on-issue reservations — `S297`/`S298` (issue #146) and `S299…S302` (issue #161) — exactly as #158 jumped over #160's; #161 landed next and reconciled this paragraph at its step-7 merge, per the sentence that used to end it.

#161 (Tier 2 at reduced ceremony — the #103 register; the deviation and its reason are in the PR body) spent **`T-WEB-S299…S301`** on `apps/web/test/accent-contrast.test.ts` — ADR-0067's family contrast arithmetic in the `T-WEB-S230` token-resolving idiom, three describes: `S299` (every game's `--ink-on-accent` ON its accent computes ≥ 4.5:1, with the recorded figures reproduced as the control), `S300` (all four resolve to a paper token, and every accent's luminance sits below the light-label ceiling derived from its own paper), `S301` (the non-text floors — each accent ≥ 3:1 against desk, and the Termo tile's `--ink` border ≥ 3:1 on the deep fill, the bound that rejected ADR-0067's darker candidates). The ids came from an **on-issue reservation** (`T-WEB-S299…S302`, posted on issue #161 before step 5); the tail **`T-WEB-S302` was the reserved review-round headroom, went unspent, and is burned below** per the rule. The re-aims in `ink-on-accent.test.ts` (`T-WEB-S72`'s termo row and title), `termo-screen.test.tsx` (the fallback pin) and `conclusion-view.test.tsx` (`T-WEB-S101`'s pair) correctly take no new id — same claims re-aimed in place, the `T-WEB-S100` burn precedent. The frontier row stands at next free `S307`, highest in use `S303`: `S297`/`S298` are #146's live on-issue reservation, jumped over exactly as #158 jumped #160's, and `S303…S306` were #162's — landed and burned while this branch was in review, reconciled here at #161's step-7 merge as the paragraph above records; #146 reconciles this paragraph when it lands.
...
| `T-WEB-S349`, `T-WEB-S350` | tails of the issue-#195 on-issue reservation — the reserved review-round headroom, unspent at step 8's exit (PR #203, `6b963d9`). Burned per the rule above; the reserving plan already gave them a meaning |
...
| `T-CORE-S6` | **predates #27.** Plan 018 reserved it for `completion-contract.test.ts` (plan 018); the assertion landed unmarked. Recorded here so the next re-derivation does not spend a pass re-investigating the gap |
| `T-WEB-S285` | tail of the issue-#58 on-issue reservation (plan 059) — the reserved review-round headroom, unspent at step 5's exit. A step-6/7 round that spends it removes it from this table (the plan-040 `T-LINT-S45` case) |
| `T-WEB-S295`, `T-WEB-S296` | tails of the issue-#158 on-issue reservation — the reserved review-round headroom, unspent at the PR's exit. Burned unspent per the rule above |
| `T-WEB-S302` | tail of the issue-#161 on-issue reservation — the reserved review-round headroom, unspent at the PR's exit. Burned unspent per the rule above |
| `T-WEB-S304…S306` | tails of the issue-#162 on-issue reservation — the reserved review-round headroom, unspent at the PR's exit. Burned unspent per the rule above |
| `T-WEB-S308` | tail of the issue-#169 on-issue reservation — the reserved review-round headroom, unspent through the Tier 1 correctness review. Burned unspent per the rule above |
...
**#145's step-7b round (the lazy-conclusion verification review) spent two fresh post-reservation ids** — the #19/#21/#31/#34 precedent for fresh ids at a review round, taken from past the frontier because #145's own `T-WEB` headroom was already spent at step 7 and its burned tails are all in other areas. **`T-WEB-S288`** (`apps/web/test/conclusion-lazy.test.tsx` — the lazy conclusion's failure story: the cold-chunk pending skeleton, the retry-once boundary, and the degraded static fallback that keeps the win from ever landing on a blank page) and **`T-WEB-S289`** (same file — every screen root warms the conclusion chunk on mount; stripping a preload effect goes red). Both sit past #58's live `S283–S285` reservation and #140's burned `S286`/`S287`, which stay burned. Widened or re-aimed in place with **no** new id, per their standing precedents: `T-LINT-S17` and `T-LINT-S16` (the free-play wall's static and dynamic door lists each gain `play/conclusion-lazy`, `nonogram/nonogram-conclusion` and `termo/termo-conclusion` — the `T-DB-9a` / `T-LINT-S39`/`S40` "a ban list gaining a name is the same claim about the same gate" precedent), and `T-WEB-S29` / `T-WEB-9c` (the pre-flush assertion in the sudoku/binairo restore tests re-aimed from "no playable board" — satisfiable by an empty container — to "a non-empty conclusion-shaped frame is present", the same claim asserted positively).
...
#58 (plan 059, ADR-0066) — **the ids were reserved on issue #58 before step 5**, the twice-earned mitigation, on the 2026-08-20 parallel night beside #142's, #145's and #140's reservations. It spent **`T-CORE-S105`** (`packages/core/test/on-time.test.ts` — `onTimeAtWrite`'s truth table: today true, one-day-back true iff seen, two-plus days false even when seen, plus the window constant's pin), **`T-CORE-S106`** (`packages/core/test/merge.test.ts` — the #58 downgrade corner pinned as ACCEPTED earliest-wins: an earlier late row beats a later credited row and the merged streak visibly drops), **`T-DB-S71…S75`** (`packages/db/test/user.test.ts`: S71 `recordSeenDay` idempotence dated by the DB clock; S72 both write arms store the supplied `onTime` verbatim and every reader projects the stored value, pinned by rows contradicting the old derivation; S73 the 0008 backfill's equivalence to the old derivation, promote-only and re-run-safe, executed from the migration file's own text, plus (step 7) the mechanical pin that 0009's sweep is the same statement; S75 the credited write's ceiling exemption pinned AT the ceiling; and `packages/db/test/merge.test.ts`: S74 the seen-days merge union — winner gets the loser's dates, overlaps conflict away, loser emptied, re-run a no-op), **`T-API-S137…S140`** (`apps/api/test/completions.test.ts`: S137 the credit end-to-end with `GET /streak` counting the day; S138 the unseen late sync stays late with streak and Dia Perfeito unmoved; S139 the multi-past-date guard's window boundary — re-aimed at step 7 (the `T-WEB-S96` rule; the record is the step-6 correctness blocker in PR #156): a straddle-shaped stale credit does not block yesterday's credit, and the predicate trips on a second in-window credited date; `apps/api/test/session.test.ts`: S140 presence at the seam — one seen row per SP day across mint, resolve and `createSessionForUser`), and **`T-WEB-S283`/`S284`** (`apps/web/test/play-sync.test.ts` — the no-client-change pin: a queued yesterday record posts the ordinary five-key body and settles on the credited 200; a 422 settles rejected with no retry). **`T-WEB-S285` was the reserved review-round headroom, unspent at step 5's exit and burned** (table above). **One fresh post-reservation id, `T-DB-S76`**, recorded here at commit time (the #63/#120 idiom, this time WITH its paragraph): the retention delete (`pruneSeenDays` keeps only today and yesterday, re-run a no-op) had no reserved slot and is a new claim, not a widening, so it takes the number past the reservation rather than a sibling letter. Widened or re-aimed in place with **no** new id, per their standing precedents: `T-DB-S24` (the repoint column list gains `on_time` — the widening its own comment always named), `T-DB-9e` and `T-DB-S5` (the user entry's four new exports; 44 names), `T-DB-S39` (the tenth table, `user_seen_days`), `T-DB-S20` (title, snapshot and fixture gain `user_seen_days` — the #145 `push_subscriptions` precedent exactly), and the derivation-era claims re-aimed at the stored column with their re-aim reasons in their titles (`T-DB-13`, `T-DB-14`, `T-DB-S13`, `T-DB-S59`, `T-DB-S60` — the `T-WEB-S96` rule: the record their old claims point at is ADR-0066).
...
#160 (Tier 1) reserved **`T-WEB-S291`/`S292`** on the issue before the fix and spent both in `apps/web/test/centered-measure.test.ts`: `S291` (the /privacidade and /estatisticas `.page` sheets declare their measure AND `margin-inline: auto`) and `S292` (the sweep — no app stylesheet's `.page` block sets a `max-width` it does not center; non-vacuity pinned on the four sheets carrying one today). Nothing burns. The `T-WEB` row above also reconciles **`S290`**, spent by #59 (`#164`, `apps/web/test/binairo-page.test.tsx`) without a row update — this paragraph's grep found it landed, the frontier-staleness case the row's own caveat predicts.

#163 (plan 065) reserved **`T-WEB-S309…S314`** — **on issue #163 before step 5**, the twice-earned mitigation, on a night when #169 (`T-WEB-S307`/`S308`) and #33 (`T-CORE-S110`/`S111`, `T-API-S155…S175`, `T-WEB-S315…S322`, `T-LINT-S52`/`S53`) reserved on their issues beside it; the frontier row above jumps all of them. Spent at step 5: **`T-WEB-S309`** (`apps/web/test/month-grid.test.ts` — the shared month-grid geometry extracted from the stats calendar: the 6-week 2026-08 worst case, `cellAt` totality with `null` as out-of-month padding only, February's leap bounds, and the pre-1970 Euclidean branch), **`T-WEB-S310`** (`apps/web/test/archive-calendar.test.ts` — the archive cell decision: linked iff in the reader's answer, killed/ragged days inert by the same absence with no second code path, month-bounds honesty, and the source-level no-clock scan with its stripper non-vacuity control) and **`T-WEB-S311`** (`apps/web/test/archive-calendar-css.test.ts` — the calendar's geometry from stylesheet text, the T-WEB-S221 idiom: the 7 × 44px column arithmetic at the mobile paddings, `aspect-ratio: 1` on all three cell kinds, uppercase scoped to `.weekday` within the calendar's own blocks, the tabular numeral register, the header/grid template identity, and `.page` declaring no `display: flex` at any width — the 21-row overflow's pin). Spent at step 7 out of the reserved review-round headroom: **`T-WEB-S312`** (`apps/web/test/archive-index.test.tsx` — the index's newest-month row window, the pin that replaces the truncation guard this ticket deleted with the day rows: a two-month straddle cuts only the newest month, a full 31-day month at exactly `NEWEST_MONTH_ROW_WINDOW` rows loses no day, an empty archive is no calendar at all, the window is `31 × GAMES.length` rather than a literal, and the page reads through that constant). The remaining tails **`T-WEB-S313`/`S314`** went unspent and **burned at step 8** (PR #175 merged as `d10e33d`, 2026-08-21; rows in the burned table above). Rewritten in place under their landed ids with **no** new number, per their standing precedents: `T-WEB-S167` (the index renders the newest month's calendar; gains the workflow-discovery arm — a non-empty index keeps a `/arquivo/mes/YYYY-MM` href, ADR-0053 decision 12's grammar as a unit pin), `T-WEB-S168` (the empty archive gains "no skeleton grid"), `T-WEB-S169` (the month page renders the grid; its all-caps loop widens from the nav to the whole page over all twelve months, and its stylesheet scan gains `.weekday`), `T-WEB-S182` (ragged days are inert cells, never placeholders) and `T-WEB-S181` (`archive-copy.test.ts`, the `T-WEB-S100` precedent — drops the deleted `recent.heading`/`dayRowAria` arms, gains the two weekday 7-tuples and the `calendar.dayAria` byte assertion; its pt-BR-literal scan covers the two new calendar source files by its existing roots). One landed probe was re-aimed in place: `T-LINT-S35`'s free-play door list names `app/arquivo/calendar-grid` where it named the deleted `day-rows` — the ban is the directory glob and the claim is unchanged.

#146 (plan 063, ADR-0068) reserved **`T-CORE S107–S109`**, **`T-DB S77–S84`**, **`T-API S142–S154`** and **`T-WEB S297–S298`** — **on issue #146 before step 5**, the twice-earned mitigation, with the frontier re-derived by the documented grep at branch time (post-#167: `S106`/`S76`/`S140`/`S294` the maxima) and again after the pre-gate `git merge origin/main`. Spent at step 5: **`T-CORE-S107`** (`cron-contract.test.ts` — `cronNotifyResponseSchema` strict, negative/fractional counts rejected) and **`T-CORE-S108`** (`notifications-contract.test.ts` — `pushNudgePayloadSchema` strict, exactly title+body, both non-empty); **`T-DB-S77…S82`** (`notify.test.ts`: S77 the `notification_sends` schema pin — columns, composite PK, channel CHECK, FK cascade; S78 `claimNudgeSend`'s write-once double-block with channels and days claiming independently; S79 the at-risk conjunction — counted-yesterday ∧ empty-today ∧ subscribed ∧ unclaimed, with the email-channel-row non-exclusion — plus the sibling **`T-DB-S79a`** (a multi-endpoint user appears exactly once; a letter, not a second S79 — the T-DB-S53a/S53b rule applied at mint time); S80 the habitual hour — median over per-counted-day EARLIEST instants, floor semantics, single-sample cold start — plus the sibling **`T-DB-S80a`** (the 21-day inclusive boundary and percentile_disc's even-count lower-middle, same rule); S81 `readTickInstant`'s one real-clock shape test, range-only; and `merge.test.ts`: S82 the ledger union — winner gets the loser's rows, a PK collision keeps the winner's `sent_at`, loser emptied, re-run a no-op); **`T-API-S142…S151`** (`cron-notify.test.ts`: S142 the extracted auth's 401 triple with publish's own suite green untouched; S143 dormancy 503 before any DB statement; S144 claim-BEFORE-send order plus `computeStreak`'s exact number in the §4.8 pt-BR copy, n=1 and n≥2; S145 two-tick idempotence; S146 a counted today claims and sends nothing; S147 404/410 pruning with the surviving sibling still sent; S148 the non-404/410 failure — row stays, claim stands, no retry; S149 the route body's strict parse plus the `cron-notify` log line; S150 one claim, serial per-row sends for a multi-subscription user; and `push-subscriptions-routes.test.ts`: S151 the ceiling — the 11th endpoint 429s, a same-user re-subscribe at the cap preserves T-API-S132's stamp claim, a cross-user repoint at the cap 429s with ownership unchanged, and an under-cap user's repoint still works); and **`T-WEB-S297`** (`fanout-cap.test.ts` — the `streak-notify.yml` scan in the S226/S233 register: hourly schedule, `-X POST`, `-fsS`, the secret bearer, zero `uses:`, `permissions: {}`). The tails **`T-CORE-S109`, `T-DB-S83`/`S84`, `T-API-S152…S154`, `T-WEB-S298`** are the reserved review-round headroom and are **burned if unspent** per the rule above; step 8 reconciles. No `T-LINT` id: the walls don't move. Widened or re-aimed in place with **no** new id, per their standing precedents: `T-DB-9e` and `T-DB-S5` (the user entry's four new exports — the two readers, the claim, and the `notificationSends` table; 48 names), `T-DB-S39` (the eleventh table, with the "ledger in the send-once sense, not the accumulable sense" note), and `T-DB-S20` (title, snapshot and fixture gain `notification_sends` — the #145/#58 precedent exactly).

#169 (Tier 1) reserved **`T-WEB-S307`/`S308`** on the issue before the fix and spent **`S307`** in `apps/web/test/hub-links-wrap.test.ts` — the hub's mobile `.secondaryLinks` block declares `flex-wrap: wrap` and an explicit `row-gap`, so five secondary links wrap instead of clipping a 390px viewport. **`T-WEB-S308` was the reserved review-round headroom, went unspent through the single correctness review, and is burned** (table above). The frontier was re-derived by the documented grep before allocating (`S303` highest in use, from #162) and the row above now sits at `S307`.

#33 (plan 064, ADR-0069) reserved **`T-CORE S110–S111`**, **`T-API S155–S175`**, **`T-WEB S315–S322`** and **`T-LINT S52–S53`** — **on issue #33 before step 5**, the twice-earned mitigation, on a night when #163 held `T-WEB-S309…S314` beside it and #169's `T-WEB-S307`/`S308` landed under it; the range starts at `S315` precisely so neither could collide, and nothing was renumbered. Spent at step 5: **`T-CORE-S110`** (`packages/core/test/telemetry-contract.test.ts` — exactly the five founding event names exist and every property schema is strict, so an extra or PII-shaped field fails the parse); **`T-API-S155…S157`** (the capture helper — a missing `POSTHOG_KEY` logs once, sends nothing and never throws; a rejecting and a hanging-then-aborted fetch both resolve silently; the posted body is the capture contract, `$process_person_profile: false` included); **`T-API-S158…S161`** (the relay route — no session is a 204 drop with nothing captured; the boundary suite 403/415/400 with nothing captured on any refusal; `archive` derived against the DB clock's São Paulo today; the happy path's resolved `userId` and full payload, plus OPTIONS); **`T-API-S162…S164`** (the `puzzle_completed` seam — a recorded win fires the full payload; an idempotent replay fires nothing; a throwing post-response task leaves the response untouched); **`T-API-S165…S170`** (all six `streak_broken` arms, including the max-counted-date guard's B1 repro and the accepted retro-close residual); **`T-API-S171`/`S172`** (`notification_opt_in` on a genuine first insert, silent on the DO UPDATE arm — `inserted`, not `stored`); **`T-API-S173`/`S174`** (`login_linked` keyed by the resolver's `winnerId`, and no email on the wire); **`T-WEB-S315…S318`** (the relay client — the session-gate buffer and its drain-once flush; rejecting, never-resolving and synchronously-throwing fetches all leaving the caller untouched; the per-`(game, date)` once-guard; the request shape and the unset-API-url loud path); **`T-WEB-S319`/`S320`** (the lifecycle seam — a fresh mount reports once with the server's date, a restored or concluded record reports nothing); **`T-WEB-S321`** (the cross-device gate — a server-claimed day reports nothing, an unclaimed day and the archive's omitted flag both report normally); and **`T-LINT-S52`** (the telemetry wall, both arms over one id — the `T-LINT-S49`/`S50` shape — measured red on six assertions before the config entries existed). **`T-CORE-S111`, `T-API-S175`, `T-WEB-S322` and `T-LINT-S53` were the reserved review-round headroom**, unspent at step 5's exit — and **the step-7 round spent three of the four**, which is the plan-040 `T-LINT-S45` case working exactly as intended: **`T-API-S175`** (`apps/api/test/telemetry-capture.test.ts` — a property the event's schema does not declare DROPS the event, with a positive control, pinning the boundary parse step-6 quality B1 found the contract module was only claiming); **`T-WEB-S322`** (`apps/web/test/no-session-replay.test.ts` — no workspace manifest and no lockfile entry resolves a replay-capable client: the INSTALL half of AC 2's "stays disabled", and the half that closes dynamic `import()` and `require()`, which no import ban can see); and **`T-LINT-S53`** (`apps/web/test/eslint-db-wall.test.ts` — the same ban as an import error at all four `webWallImportPatterns` repeaters plus `apps/api` and `packages/**`, measured red at both halves before the config entries existed, with an anti-vacuity control that the list is exact names and not a `posthog*` substring: the SOURCE half). `T-CORE-S111` stayed unspent and **burned at step 8** (PR #176 merged as `651ec33`, 2026-08-21; row in the burned table above) — the only one of this reservation's four tails to burn. `T-WEB-S321` is the case plan 064 §S4 reserved conditionally — the ADR-0065 remote-completed view DOES mount `usePlayLifecycle`, so the conditional arm fired; **what it pins is not what the plan drafted**: the claim travels into the hook as the `remotelyClaimed` input rather than being read there, because `play/day-state.ts` reaches `src/day/**` and no archive page's module graph may contain either (ADR-0053 decision 10's "Why no endpoint" — **not** decision 9, corrected at step 7 per step-6 ADR B1; `T-WEB-S183`'s scan — which went red on the first attempt and is the reason for the shape). No id was widened in place.

#205 (the comment-density sweep) spent **`T-WEB-S355`** in `apps/web/test/session-bootstrap.test.tsx`: a failed mint — rejected fetch, 500, or a 200 the contract refuses — still opens the telemetry gate, so buffered `puzzle_started` events are never held forever. It is the sweep's one genuinely uncovered rule: `bootstrap.ts`'s doc block was its only copy, and the whole suite stayed green with the gate mutated to fire on success alone. Nothing reserved, nothing burned.
