...
**Amended by:** [ADR-0070](./0070-the-daily-nonogram-conclusion-names-its-motif.md) (#64) — decision 1's honest-absence table gains the motif **name** as a second recorded absence on the Nonogram row (the picture's absence is untouched and still exact), and decision 2's one-wire-field template is the one `motifName` is built to. Both annotated in place; nothing deleted.
...
1. **Evidence-bounded scope — full where the server holds the data, honest
   ...
   **Full Termo (guess grid + answer word) requires new server storage or a
   ...
   > **The Nonogram row gains a SECOND named absence at #64 ([ADR-0070](./0070-the-daily-nonogram-conclusion-names-its-motif.md) decision 7): the motif NAME.** The picture's absence row above is untouched and still exact — the bitmap is solution content and stays off this wire. The name is a different case and is recorded rather than shipped: `motifName` IS on the claim this view reads, and this view deliberately does not read it. *"Você revelou: Âncora"* with **no picture**, on a device where the player did nothing, is a caption for a missing image. An unread optional claim field is exactly what this table is for; a later ticket can reverse the cut cheaply, and nothing in the design has to change for it to.
...
2. **One wire field: optional `hintsUsed` on the per-game claim**, the #141
   ...
   > **This decision is the TEMPLATE #64 built to ([ADR-0070](./0070-the-daily-nonogram-conclusion-names-its-motif.md) decision 1), and it is cited here because the failure mode above is the one that shaped it.** The claim gains an optional `motifName` on exactly this shape: optional, a value on a non-`completed` claim is a parse failure, and the producer scopes it (to **nonogram** here, as this decision scopes `hintsUsed` away from Termo). The 500-the-whole-payload warning in this decision is why `getPublishedNonogramMotifName` normalises a blank stored name to `undefined` at the read: the wire carries `.min(1)`, the route `parse`s its own output, and one row with `reveal.name: ""` would otherwise take down the hub, the four tiles and this view together — verbatim the `hintsUsed`-cap failure recorded above. One difference is worth naming: `hintsUsed` projects a stored NOT NULL column of the user's own row, while `motifName` comes from `daily_puzzles`, so `/day` gained its first read of published CONTENT — conditional, and behind a helper that never throws.
