...
**Defect closed by:** [#63](https://github.com/fernandolisboa/miolos/issues/63), 2026-08-19 — **decision 5 is written in the present tense and its present tense is no longer true.** `.timerBar` and `.timerCard` now carry `var(--font-ui)`; the running clock shifts on no play screen. Decision 5 is left as written because what it records is the decision *not to fix it inside #25*, which is history rather than a live claim; this line is what stops a reader taking its measurement's tense for the state of the code. Nothing here is amended — this is decision 1 being applied. Swept with it and left on Fraunces deliberately, all decision 2's case: the Binairo and Sudoku board cells, `app/page.module.css`'s `.streakNumeral`, and `conclusion-view.module.css`'s `.stampTime`, which renders a frozen value. Gate: `T-WEB-S230`.
