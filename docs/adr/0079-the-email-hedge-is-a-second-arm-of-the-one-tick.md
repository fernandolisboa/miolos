...
**Amends:** [ADR-0068](./0068-the-dispatchers-operating-decisions.md) decisions 4 and 5, and its consequence that the push reader stays untouched; [ADR-0050](./0050-email-attach-magic-link-tokens-consents-and-the-lgpd-minimum.md) decisions 7 and 11, by discharging their revisit triggers
...
6. **Withdrawal is by mail, and it sets `users.reminder_consent_at` to NULL.** The email says how to stop and carries `List-Unsubscribe: <mailto:privacidade@miolos.app>`. This is v1's first withdrawal surface, so it discharges ADR-0050 decision 7's trigger. NULL already means "not consented". ADR-0012's given/withdrawn columns wait for the first self-service surface. Running the SQL is Fernando's (`docs/pending-fernando.md`).
