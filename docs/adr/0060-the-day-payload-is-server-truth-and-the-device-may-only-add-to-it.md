...
**Amended by:** [ADR-0065](./0065-a-cross-device-done-day-opens-a-completed-view.md) (#142, Fernando's answer to PR #135 veto decision 3) — **(f) and (g), continuing the series; both decision-level.** Annotation (f): decision 2's per-game claim gains an optional `hintsUsed`, on annotation (b)'s template exactly. Annotation (g): **decision 8's playable-board sentence is replaced for the daily path** — a cross-device done day now opens a completed view — while the decision's href sentence **stands untouched**, which is why this is `Amended by` and not `Superseded in part by`. Every edit annotated in place, nothing deleted; references qualify the letter ("annotation (f)"), because this ADR's native Consequences are lettered too.
