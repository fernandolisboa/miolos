...
6. **The email is stopped by a checkbox in Ajustes, never by writing an email.** Fernando's call on 2026-09-24. The checkbox and its `POST /account/reminder-consent` are #36's second pull request (ADR-0082), which records the withdrawal time. This arm reads only `reminder_consent_at IS NOT NULL`, so it needs no change when that lands. Every email links to `/ajustes`, and `List-Unsubscribe` points there too. **This arm was to wait for that checkbox; on 2026-09-25 Fernando shipped it first, while he was its only recipient.** Rejected: a mailbox the operator reads and acts on by hand.
