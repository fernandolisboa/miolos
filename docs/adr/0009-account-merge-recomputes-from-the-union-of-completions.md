...
**Amended by:** [ADR-0066](./0066-a-late-sync-is-credited-from-a-server-seen-day.md) — the queued #58 amendment, additive as promised. Seen days (`user_seen_days`) are a **write-time-only** input: the table is consulted exactly once per completion, at write time, inside `POST /completions`; no streak, statistics, medal or day-state computation reads it, and the pure merge recompute (`mergeCompletions`) never consults it — `mergeAccounts` only UNIONS its rows, computing nothing — so *"a streak is always derivable from completion rows"* stays exactly true. That constraint is in fact what FORCED on-time to be decided at write and stored (ADR-0066 decision 2). This ADR's conclusion is unchanged.
