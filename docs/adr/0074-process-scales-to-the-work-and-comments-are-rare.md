...
- **Removing ~12,500 comment lines cannot be one PR.** It runs in per-package tranches. Six comments encode invariants no test covers (the `CRON_SECRET` operational runbook, two `vitest.shared.ts` constraints, the PGlite `t0` ordering, the nonogram oracle ban, the share-button dead-control residual) and must be relocated before they are cut. **Discharged by [ADR-0075](./0075-the-comment-budget-is-a-tool-not-a-rule.md):** each is now a test, an ADR line or `apps/api/.env.example`, and the budget that replaced this decision's prose is enforced by `pnpm comments`.
