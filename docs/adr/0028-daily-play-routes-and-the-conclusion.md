...
**Amended by:** [ADR-0071](./0071-the-archive-cards-are-nameplates-and-two-leave-the-file-convention.md) — **decision 5's per-route enumeration of the ADR-0014 direct-read extension grows again, from eight to ten, and it grows by NAMING rather than by implication.** #104 adds `app/cartao/[data]/route.ts` and `app/cartao/mes/[mes]/route.ts`: public, unauthenticated, `force-dynamic` reads through a wall helper (`listArchivedDays`), whose audience is a social scraper rather than a browser, exactly the class the #34 annotation describes. That annotation's own rule — *"The enumeration is per-route, so it does not cover a metadata image route by implication either"* — is what makes this amendment owed: a route is covered only if it is named, and these two are **route handlers**, not metadata routes, so nothing about #34's wording reaches them. *"Eight is the whole count"* becomes **ten**; the committed `app/arquivo/opengraph-image.png` is still not a route and still does not count, for the reason that sentence already gives about the site card. Decision 4's segment count is untouched (`/cartao` is not under `/<jogo>`), decision 6's `generateStaticParams` prohibition is obeyed, and the fifth-game recipe at decision 5's consequence is **unchanged** — #104's cards are not per-game. See the third annotation at decision 5. Multiple `Amended by:` lines stack.
...
5. **ADR-0014's scope is extended, explicitly.** `apps/web` reads
   ...
   *(**Grown again at #104** —
   ...
   decisions 3 and 4. **Ten, not eight.** The enumeration grows by two more
   files, both public, unauthenticated, `force-dynamic` reads through a wall
   helper — `app/cartao/[data]/route.ts` and `app/cartao/mes/[mes]/route.ts`,
   both on `listArchivedDays(db, {…, limit: 1})`, used only as
   `length === 0 ? 404 : render`. **They are named because the paragraph above
   says the enumeration does not cover by implication**, and these two are
   **route handlers** rather than metadata image routes, so #34's wording
   reaches them even less than it reached the eight. They inherit that
   paragraph's own posture whole: the read is an existence proof, the response
   carries `private, no-cache, no-store` on the 404 arm as well as the 200,
   and Zod parses the segment before any read. The committed
   `app/arquivo/opengraph-image.png` is still not a route and still does not
   count, for the reason the site-card clause above already gives.)*
