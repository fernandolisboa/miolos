...
**Amended at #146** ([plan 063](../plans/063-issue-146-plan-streak-at-risk-dispatcher.md), [ADR-0067](./0067-the-dispatchers-operating-decisions.md)) — annotations (a)–(c) below; none decision-level: each resolves a point this ADR itself left open to #146 (Q2's confirmable default, decision 10's open call, consequence (d)'s open Q1)
...
3. **The prompt has one lifecycle per account, server-stamped.**
   ...
   *annotation (a), #146, 2026-08-20: CONFIRMED, Q2 = 2a on #32; the
   dispatcher's candidate SQL carries no `>= 3` gate anywhere, ADR-0067*).
   THE ASK
...
10. **Endpoint hygiene rides the send pass** (slice B): a 404/410 from the
    ...
    *Annotation (b) (#146, 2026-08-20): the call is made — DEFERRED to #36,
    the settings-toggle ticket (ADR-0067 decision 6, and the comment posted
...
- (d) The email hedge (slice C) shares the dispatcher and the ledger;
  ...
  without touching this one (ADR-0067).*
