...
- **Do (Fernando, one paste in a real terminal — both writes are classifier-blocked for an agent):**
  ...
  ```
  **Where to get the token:** PostHog → **Settings → Project → General**, the *Project API key* field. The value starts with **`phc_`**. (The top-of-page banner's "grab your project API key from Project Settings" link lands in the same place.) Do not chase the value through Vercel: `vercel env pull` would dump miolos-web's entire production environment — `WEB_DATABASE_URL` included — to read one publishable token, and the dashboard's eye icon may refuse if the variable is Sensitive (the `CRON_SECRET` lesson).
  ⚠️ **NOT the "Project secret API keys" page**, which is a different credential and a trap this item set once already (Fernando, 2026-08-21, landed there following an earlier wrong path in this file). A **project secret API key** (`phs_`) grants server-to-server **read** access to project data and is genuinely secret; the **project API key** (`phc_`) is write-only — it can create events and cannot read anything — which is why it is safe in public bundles. `capture.ts` posts to the capture endpoint with `api_key`, so `phc_` is the one. Creating a `phs_` key here would mint a new credential nothing needs. (A third animal, the **personal** API key, is what the SOON #37 deletion row would need — the publishable token cannot delete. Still not this.)
  **Answer "not sensitive" at Vercel's prompt.** The `phc_…` token is publishable by design and ADR-0069 keeps it off *our* client for architectural reasons, not secrecy. Non-sensitive keeps it readable for later verification instead of re-creating the `CRON_SECRET` dead end, at no security cost.
