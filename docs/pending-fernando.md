...
> **SETTLED 2026-08-21 (day): the value is unreadable, and the copy is off the table for good.** Fernando ran `vercel env pull` and read the file by eye: the line is present, and its value is the literal string `[SENSITIVE]`. `CRON_SECRET` is marked **Sensitive** on miolos-api — write-only by design. `vercel env run -e production` agrees, delivering it **empty** (verified twice, the second time with `apps/api/.env.local` moved aside to rule out local shadowing).
>
> **The `grep -c` test this item carried for one commit was wrong** and is deleted: it counts the *line*, which is present either way, so it prints `1` even when the value is a placeholder. Following it into the next line would have set the GitHub secret to the literal seven characters `[SENSITIVE]` — the tick would have gone on 401ing while the secret's timestamp updated to look like a successful fix. **Never test readability by the presence of the key; read the value.**

**Rotation is the only fix.** Generate a new value and write it to both holders, because neither can be read into the other.

- **Do (Fernando, one paste in a real terminal — an agent is classifier-blocked from both writes):**
  ```
  cd ~/projects/miolos/apps/api
  new=$(openssl rand -hex 32)
  printf '%s' "$new" | gh secret set CRON_SECRET
  printf '%s' "$new" | vercel env add CRON_SECRET production --force --sensitive -y
  unset new
  ```
  `--force` overwrites in place, so there is no window with the variable absent. `--sensitive` keeps the classification as it was: the rotation pattern never needs to read the value back, so nothing is gained by weakening it.
- **Then (agent):** land a commit that touches `apps/api/**` to force the production build. This is not optional and a plain `vercel --prod` is not enough — `apps/api/vercel.json` sets `"ignoreCommand": "npx turbo-ignore"`, so a redeploy with nothing changed under `apps/api` is **skipped** (those 6-second `Canceled` rows in `vercel ls miolos-api --prod`) and the new value never lands. Whether `vercel --prod --force` defeats the ignore step is undocumented; a real diff under `apps/api` is deterministic, so use that.
- **Timing:** the env write and the redeploy must happen in one sitting. Between them, Vercel's cron sender has the new value while the deployed route still checks the old one, so **`/cron/publish` would 401** — stay well clear of its `0 6 * * *` UTC tick.
