...
- **Under (a), each time a stop request arrives:** a sender address can be forged, so first reply to that address and wait for its owner to confirm. Then run via the operator ritual (napkin § Shell 2): `update users set reminder_consent_at = null where email = lower(trim('<address>')) and email_verified_at is not null;` — expect `UPDATE 1`. The next hourly tick no longer picks that account.
...
- **Blocks:** the timing, not the code. Merging #199's pull request turns the emails on at the next hourly tick (Resend is already live). If your answer is (b), wait to merge until #267 is built.
