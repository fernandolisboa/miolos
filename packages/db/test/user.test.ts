...
describe("listCompletionsForDay (#83, ADR-0060)", () => {
  it("T-DB-S59: only that user's rows for exactly that date, with the STORED onTime (#58)", async () => {
    ...
    // The projection is exactly the five fields the day projection reads
    // (`elapsedMs` since #141, `hintsUsed` since #142, both for
    // `dayGamesFromRows`) — still no `completedAt` on this wire path.
  ...
  it("T-DB-S65: the stored duration and hint count are projected back exactly, per row (#141, widened at #142)", async () => {
    ...
    // `hintsUsed` joined the claim at #142 with no new id (the T-DB-9a
    // precedent: the same claim about the same projection, one field wider) —
    // 1 and 0 both, so "projected" is pinned rather than "defaulted".
    const userId = await createUser();
    const today = await todaySaoPaulo(ctx.db);
