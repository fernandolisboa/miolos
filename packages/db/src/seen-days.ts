 ...
 * (T-DB-S71). The composite PK makes it idempotent — `ON CONFLICT DO
 * NOTHING`, the completions pattern — and the DB clock names the day: no
 * date crosses this seam at all.
 *
...
export async function recordSeenDay(db: Db, userId: string): Promise<void> {
  await db.execute(
    sql`insert into ${userSeenDays} (user_id, date)
        values (${userId}::uuid, (now() at time zone ${SAO_PAULO_TIME_ZONE})::date)
 ...
 * (the constant's comment in packages/core names this edit).
 */
