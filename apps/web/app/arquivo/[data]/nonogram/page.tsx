...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: { canonical: archiveGameRoute(date, "nonogram") },
    openGraph: { ...OG_DEFAULTS, title, description },
  };
}

/**
 * The archived Nonogram for one past day (#31 AC 1).
 *
 ...
 */
export default async function ArchiveNonogramPage({ params }: PlayPageProps) {
  const date = parseArchiveDate((await params).data);
  if (date === undefined) {
    notFound();
  }
  ...
  const db = getDb();
  const daily = await getArchivedDaily(db, "nonogram", date);
  if (daily === undefined) {
    const dateClass = await archiveDateClass(db, date);
    if (dateClass === "today") {
      redirect(routes.nonogram);
    }
    notFound();
  }

  return <ArchiveNonogramScreen daily={daily} />;
}
