...
/**
 * The client half of GET /streak (ADR-0048, plan 027 §8) — fetch and parse
 * only, no React, so it is testable without rendering. The value is
 ...
 * zero state (ADR-0031 decision 6: device state can never back a streak).
 *
 ...
 * (eslint.config.mjs, ADR-0046) — free play never touches the streak.
 */
export async function fetchStreak(): Promise<StreakResponse | undefined> {
  ...
  // fetch would hit the relative URL "undefined/streak" and the catch below
  ...
  try {
    ...
    // Swallow network errors: an offline hub must not break, it reads 0.
    return undefined;
  }
}
