/**
 * The React seam over the pure reducer (plan 018 §8, §10): the solution
 ...
 * `usePlayLifecycle` (ADR-0029, plan 018 §5.4).
 *
 * The one rule that carries over from Binairo unchanged: **nothing here
 * runs during render** (plan 017 D28). `localStorage` is read once, in the
...
export interface SudokuPlay {
  ...
  /** Which case the last hint fired, for the one-line explanation (§12.4). */
   ...
   * `End` are `moveSelection(0, -8)` and `moveSelection(0, 8)` (§8.4).
   */
...
export function useSudokuPlay(
  ...
  // is not nullish (S6, landmine 22).
  const playable = useMemo(() => playableGivens(givens), [givens]);
  const filled = countFilled(playable, entries);

  // Measured 0.057 ms mean / 0.52 ms max on the hardest daily (§10.1),
  // memoized once per page. `null` is unreachable for a uniquely-solvable
...
/**
 * The one place a Sudoku `PlayRecord` is constructed (§9.1), handed to the
 ...
 * with no empty cell left (§8.2). `elapsedMs` is clamped by
